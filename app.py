...
async def start():
    ...
    logger.info("JSON data: %s", json.dumps(json_data, indent=2))

    # Call Predictor API
    prediction_result = call_predictor_api(json_data)
    logger.info("Prediction result: %s", json.dumps(prediction_result, indent=2))

    # Generate Report and Table
    report, table_df = generate_report_and_table(prediction_result, csv_file.path)
    logger.info("Report: %s", report)
    logger.info("Table: %s", table_df)
    ...
    try:
        await cl.Message(
            content="Test message to check if messages are being displayed." # f"`{csv_file.name}` uploaded and processed."
        ).send()
        logger.info("Upload and process message sent successfully.")
    except Exception as e:
        logger.error("Error sending upload and process message: %s", e)
    ...
    try:
        ...
        logger.info("Report and table message sent successfully.")
    except Exception as e:
        logger.error("Error sending report and table message: %s", e)
